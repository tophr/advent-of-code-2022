...
function sumSizeTotal(obj) {
    // Base case: if the object is not an object or is empty, return 0
    if (typeof obj !== "object" || !obj) return 0;
  
    // Recursive case: sum the sizeTotal property of the current object,
    // and call sumSizeTotal on all of its nested objects
    return (obj.sizeTotal || 0) +
      Object.values(obj)
            .map(sumSizeTotal)
            .reduce((a, b) => a + b, 0);
}
...
const realTotal = sumSizeTotal(filesystem) - totalUsed - totalUsed;
const totalFree = diskSpace - realTotal;
